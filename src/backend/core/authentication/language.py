...
def compute_language(user_info):
    ...
    (written to Zitadel's standard ``locale`` claim); calendars only reads
    it. The claim is a BCP47 tag (e.g. "nl", "nl-NL" or "en"); only its
    primary subtag is used, matched against the primary subtag of each code
    in ``settings.LANGUAGES`` (e.g. "en" -> "en-us", "nl" -> "nl-nl").
    ...
    supported_languages = {
        code.split("-")[0]: code for code, _name in settings.LANGUAGES
    }
